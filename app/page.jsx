...
export default function Home() {
  ...
  const fetchData = async (page = 1) => {
    try {
      ...
      if (responseData?.data) {
        ...
        // Map trailer data directly from the list response to avoid N+1 fetches
  ...
  const toggleTrailer = useCallback((id) => {
    setActiveTrailer((prev) => (prev === id ? null : id));
  }, []);
      ...
      >
        {animeData.map((anime, index) => (
          <AnimeCard
            key={anime.mal_id}
            anime={anime}
            isActiveTrailer={activeTrailer === anime.mal_id}
